  ...
  return (
    <section className="text-gray-600 body-font bg-yellow-200">
      <div className="container px-5 py-24 mx-auto bg-slate-50 ">
        <div className="flex flex-wrap -m-4 bg-slate-50 ">
          {
            products.map((product) => {
              console.log(product, 'product')
              const { id, title, price, category, image } = product;
              return (
                <Link to={`/products/${id}`} className="lg:w-[23%] md:w-1/2 p-4 w-full mb-4 cursor-pointer rounded-lg shadow ml-4 hover:scale-110">
